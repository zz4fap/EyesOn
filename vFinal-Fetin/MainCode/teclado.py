...
def deletar():
    pag.press("backspace") #deleta no google
    entry.delete(len(entry.get()) - 1) #deleta na interface
...
def setup_gui():
    ...
    btn_backspace = ttk.Button(root, text="Deletar", command=deletar, style="TButton") #TENTAR USAR O PYAUTOGUI PARA DELETAR A LETRA
    btn_backspace.grid(row=5, column=6, columnspan=2, padx=5, pady=5, sticky="nsew") #pag.press("backspace")
    #entry.delete(len(entry.get()) - 1)
    #ALTERADA FUNÇÃO DELETAR
...
def run_keyb_thread():
    global keyb_thread_on
    pag.moveTo(80, 255)
    run_kb = threading.Thread(target=setup_gui)
    run_kb.start()
    keyb_thread_on = True
